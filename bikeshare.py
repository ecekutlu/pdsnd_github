...
def get_city():
    """Asks the user for a city and returns bike share data."""
    city = ''
    while city.lower() not in ['chicago', 'new york', 'washington']:
        ...
        if city.lower() == 'chicago':
            ...
            print('Oops! That was not quite right, go again!')

def get_time_period():
    """Asks the user for a time period and returns the specified filter"""
    time_period = ''
    while time_period.lower() not in ['month', 'day', 'none']:
        time_period = input('\nHow would you like to filter your data by month, day,'
                            ' or no filter? Input "none" for no time filter.\n')
        if time_period.lower() not in ['month', 'day', 'none']:
            print('Oops, I do not get that. Let\s try again')
    return time_period
...
def get_month():
    ...
    while month_input.lower() not in months_dict.keys():
        month_input = input('\nWhich month between January and June?\n')
...
def get_day():
    """ Asks for a day as input of an integer"""
    this_month = get_month()[0]
    month = int(this_month[5:])
    valid_date = False
    while valid_date == False:
        is_int = False
        day = input('\nWhich day? Please input an integer.\n')
        while is_int == False:
            try:
                day = int(day)
                is_int = True
            except ValueError:
                print('Oops, I do not get that. Let\'s try again. Please type your'
...
def popular_hour(df):
    ...
    print('The most popular hour o for start time is {}{}.'.format(pop_hour_readable, am_pm))

def trip_duration(df):
    """Finds the total trip duration and average trip duration in."""
...
def popular_stations(df):
    """Prints the most popular start station and end station."""
...
def popular_trip(df):
    ...
    most_pop_trip = df['enroute'].mode().to_string(index = False)
    # The 'enroute' column is created in the statistics() function.
...
def birth_years(df):
    """Prints eldest, youngest and most popular age group."""
    earliest = int(df['birth_year'].min())
    latest = int(df['birth_year'].max())
    mode = int(df['birth_year'].mode())
    print('The eldest bikers were born in {}.\nThe youngest bikers are born in the year {}.'
...
def display_data(df):
    ...
    while valid_input == False:
        display = input('\nWould you like to view individual trips? '
                        'Type \'yes\' or \'no\'.\n')
        valid_input = is_valid(display)
        if valid_input == True:
            break
        else:
            print("Oops I don\'t get that. Let\'s try again. Type yes or no")
    if display.lower() == 'yes':
        # prints every column except the 'enroute' column created in statistics()
...
def statistics():
    ...
    # creates a 'enroute' column that concatenates 'start_station' with
    # 'end_station' for the use popular_trip() function
    df['enroute'] = df['start_station'].str.cat(df['end_station'], sep=' to ')
    ...
    while restart.lower() not in ['yes', 'no']:
        print("Oops no such answer! Please type 'yes' or 'no'.")
        restart = input('\nWould you like to go again? Type \'yes\' or \'no\'.\n')
